...
// TUS DATOS CLOUDINARY (Solo para posts ahora)
...
export default function Home() {
  ...
  const uploadImage = async (file) => {
    ...
    } catch (err) { setIsUploading(false); alert("Error subida"); return null; }
  };

  const handlePostFileChange = async (e) => {
    const file = e.target.files[0];
    if (file) { const url = await uploadImage(file); if (url) setImgUrl(url); }
  };
  ...
  // IR A PERFIL (Esto reemplaza la subida de foto en navbar)
    ...
    <div className="home-container">
      <div className="navbar">
        ...
        <div className="search-bar-container" style={{position: "relative"}}>
          ...
          {searchResults.length > 0 && <div className="search-results">{searchResults.map(u => <div key={u._id} className="search-item" onClick={() => goToProfile(u.username)}><div style={{display:"flex", alignItems:"center", gap:"10px"}}><img src={u.profilePic || "https://cdn-icons-png.flaticon.com/512/149/149071.png"} alt="" style={{width:"30px", height:"30px", borderRadius:"50%", objectFit:"cover"}}/><span>{u.username}</span></div>{u._id !== user._id && <button className={`mini-follow-btn ${user.followings.includes(u._id) ? "following-mode" : ""}`} onClick={(e) => { e.stopPropagation(); handleFollow(u._id); }}>{user.followings.includes(u._id) ? "Siguiendo" : "Seguir"}</button>}</div>)}</div>}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          ...
          {/* ESTO CAMBIÓ: AHORA SOLO TE LLEVA AL PERFIL */}
          <div onClick={() => goToProfile(user.username)} style={{cursor: "pointer", display:"flex", alignItems:"center", gap:"5px"}} title="Ir a mi perfil">
          ...
          <div className="share-box">
            ...
            <div className="share-bottom" style={{flexDirection:"column", alignItems:"flex-start", gap:"10px"}}>
              <div style={{display:"flex", justifyContent:"space-between", width:"100%", alignItems:"center"}}>
                ...
                <button className="share-btn" onClick={handleSubmit} disabled={isUploading}>{isUploading ? "Subiendo..." : "Publicar"}</button>
              </div>
              {imgUrl && <div style={{position:"relative", width:"100%"}}><img src={imgUrl} alt="Preview" style={{width:"100%", maxHeight:"300px", objectFit:"contain", borderRadius:"5px", border:"1px solid #333"}} /></div>}
            </div>
          </div>
