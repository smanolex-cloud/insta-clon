...
export default function Home() {
  ...
  const [hasMore, setHasMore] = useState(true); // ¿Quedan fotos por cargar?
  const [isFetching, setIsFetching] = useState(false); // Evitar cargar doble
  ...
  // 1. CARGA INICIAL (Página 1) + Otros datos
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        // Cargar página 1 de posts
        const postsRes = await axios.get(`${API_URL}/posts/timeline/all?page=1&limit=5`);
  ...
  // 2. DETECTAR SCROLL AL FONDO
  useEffect(() => {
    const handleScroll = () => {
      // Si la suma de lo que hemos scrolleado + la altura de la ventana >= altura total del documento - 10px
  ...
  // 3. CARGAR MÁS POSTS CUANDO CAMBIA LA PÁGINA
  useEffect(() => {
    if (page === 1) return; // La 1 ya se cargó al inicio

    const fetchMorePosts = async () => {
      setIsFetching(true);
      try {
        const res = await axios.get(`${API_URL}/posts/timeline/all?page=${page}&limit=5`);
        
        if (res.data.length === 0) {
          setHasMore(false); // Ya no hay más fotos en el servidor
        } else {
          setPosts((prev) => [...prev, ...res.data]); // Pegamos las nuevas abajo
        }
      } catch (err) { console.error(err); }
      setIsFetching(false);
    };
  ...
  const uploadImage = async (file) => {
    ...
    try { const res = await axios.post(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/image/upload`, data); setIsUploading(false); return res.data.secure_url; } catch (err) { setIsUploading(false); alert("Error al subir imagen."); return null; }
  };
    ...
    <div className="home-container">
      <div className="navbar">
        ...
        <div className="search-bar-container desktop-only" style={{position: "relative"}}>
          ...
          {searchResults.length > 0 && <div className="search-results">{searchResults.map(u => <div key={u._id} className="search-item" onClick={() => goToProfile(u.username)}><div style={{display:"flex", alignItems:"center", gap:"10px"}}><img src={u.profilePic || DEFAULT_IMG} alt="" style={{width:"30px", height:"30px", borderRadius:"50%", objectFit:"cover"}}/><span style={{fontWeight:"bold"}}>{u.username}</span></div>{u._id !== user._id && <button className={`mini-follow-btn ${user.followings.includes(u._id) ? "following-mode" : ""}`} onClick={(e) => { e.stopPropagation(); handleFollow(u._id); }}>{user.followings.includes(u._id) ? "Siguiendo" : "Seguir"}</button>}</div>)}</div>}
        </div>
        <div className="desktop-only" style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          <div className="notification-container" style={{position: "relative"}}><span onClick={handleNotiClick} style={{fontSize: "24px", cursor: "pointer"}}>🔔</span>{bellCount > 0 && <span className="noti-badge">{bellCount}</span>}{showNotiPanel && <div className="noti-dropdown">{notifications.filter(n => n.type !== 'message').length === 0 ? <p style={{padding:"10px", fontSize:"12px"}}>Sin actividad</p> : notifications.filter(n => n.type !== 'message').map(n => <div key={n._id} className="noti-item"><strong>{n.senderName}</strong> {n.type==='like' && "❤️ like"}{n.type==='follow' && "🤝 te sigue"}{n.type==='comment' && "💬 comentó"}{n.type==='commentLike' && " ❤️ le gustó tu comentario"}</div>)}</div>}</div>
        ...
        <div className="mobile-only" style={{position:"relative"}}><button onClick={handleChatClick} className="chat-btn">💬</button>{msgCount > 0 && <span className="noti-badge-chat" style={{top: "-5px", right: "-5px"}}>{msgCount}</span>}</div>
      </div>
      ...
      <div className="main-content">
        <div className="feed-container">
          ...
          {isFetching && <p style={{textAlign:"center", padding:"10px", color:"gray"}}>Cargando más...</p>}
          {!hasMore && <p style={{textAlign:"center", padding:"10px", color:"#333"}}>Has llegado al final 🏁</p>}
        </div>
        
        <div className="rightbar">
          <h3>Sugerencias</h3>
          <ul className="user-list">{users.map((u) => (<li key={u._id} className="user-item"><div style={{display:"flex", alignItems:"center", gap:"10px"}}><img src={u.profilePic || DEFAULT_IMG} alt="" style={{width:"30px", height:"30px", borderRadius:"50%", objectFit:"cover"}} /><span style={{fontWeight:"bold", cursor: "pointer"}} onClick={() => goToProfile(u.username)}>{u.username}</span></div><button className={`follow-btn ${user.followings.includes(u._id) ? "following-mode" : ""}`} onClick={() => handleFollow(u._id)}>{user.followings.includes(u._id) ? "Siguiendo" : "Seguir"}</button></li>))}</ul>
        </div>
      </div>

      <div className="mobile-navbar">
        <button className="mobile-nav-item" onClick={() => window.location.href="/"}>🏠</button>
        <button className="mobile-nav-item" onClick={mobileSearch}>🔍</button>
