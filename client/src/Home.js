...
export default function Home() {
  ...
  const uploadImage = async (file) => {
    ...
    try { const res = await axios.post(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/image/upload`, data); setIsUploading(false); return res.data.secure_url; } catch (err) { setIsUploading(false); alert("Error subida"); return null; }
  };
    ...
    <div className="home-container">
      <div className="navbar">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          <div className="notification-container" style={{position: "relative"}}>
            ...
            {showNotiPanel && (
              <div className="noti-dropdown">
                {notifications.filter(n => n.type !== 'message').length === 0 ? <p style={{padding:"10px", fontSize:"12px"}}>Sin actividad</p> : notifications.filter(n => n.type !== 'message').map(n => (
                  <div key={n._id} className="noti-item">
                    <strong>{n.senderName}</strong> 
                    {n.type === 'like' && " ❤️ le dio me gusta a tu post"}
                    {n.type === 'comment' && " 💬 comentó tu post"}
                    {n.type === 'follow' && " 🤝 te empezó a seguir"}
                    {/* AQUÍ MOSTRAMOS EL LIKE AL COMENTARIO */}
          ...
          <div onClick={() => profileInputRef.current.click()} style={{cursor: "pointer", display:"flex", alignItems:"center", gap:"5px"}}><img src={user.profilePic || DEFAULT_IMG} alt="" style={{width:"35px", height:"35px", borderRadius:"50%", objectFit:"cover", border:"1px solid #555"}}/><span style={{ fontWeight: "bold", color: "#fff" }}>{user.username}</span><input type="file" ref={profileInputRef} style={{display:"none"}} onChange={handleProfileFileChange} accept="image/*" /></div>
      ...
      <div className="main-content">
        ...
        <div className="rightbar">
          ...
          <ul className="user-list">
            {users.map((u) => (
              <li key={u._id} className="user-item"><div style={{display:"flex", alignItems:"center", gap:"10px"}}><img src={u.profilePic || DEFAULT_IMG} alt="" style={{width:"30px", height:"30px", borderRadius:"50%", objectFit:"cover"}} /><span style={{fontWeight:"bold", cursor: "pointer"}} onClick={() => goToProfile(u.username)}>{u.username}</span></div><button className={`follow-btn ${user.followings.includes(u._id) ? "following-mode" : ""}`} onClick={() => handleFollow(u._id)}>{user.followings.includes(u._id) ? "Siguiendo" : "Seguir"}</button></li>
            ))}
          </ul>
        </div>
      </div>
